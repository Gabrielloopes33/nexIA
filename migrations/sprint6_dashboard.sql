-- Sprint 6: Dashboard e Métricas
-- Criação da tabela dashboard_metrics e view de resumo
-- Data: 2026-03-13
...
CREATE TABLE IF NOT EXISTS dashboard_metrics (
    ...
    -- Constraint única para evitar duplicatas de métricas do mesmo período
...
-- Índice para busca por organização (mais frequente)
CREATE INDEX IF NOT EXISTS idx_dashboard_metrics_organization_id 
    ON dashboard_metrics(organization_id);

-- Índice para busca por chave de métrica
CREATE INDEX IF NOT EXISTS idx_dashboard_metrics_metric_key 
    ON dashboard_metrics(metric_key);

-- Índice para busca por data de referência
CREATE INDEX IF NOT EXISTS idx_dashboard_metrics_reference_date 
    ON dashboard_metrics(reference_date);

-- Índice composto para queries de dashboard (organização + chave + data)
CREATE INDEX IF NOT EXISTS idx_dashboard_metrics_org_key_date 
    ON dashboard_metrics(organization_id, metric_key, reference_date DESC);

-- Índice para busca por período
CREATE INDEX IF NOT EXISTS idx_dashboard_metrics_period 
    ON dashboard_metrics(metric_period);

-- Índice GIN para metadata JSONB
CREATE INDEX IF NOT EXISTS idx_dashboard_metrics_metadata 
...
-- Agregação em tempo real dos principais indicadores
-- ============================================
...
-- ============================================
-- SEED DATA: Métricas iniciais para cada organização
-- ============================================
...
COMMENT ON TABLE dashboard_metrics IS 'Tabela de métricas agregadas do dashboard por organização';
COMMENT ON COLUMN dashboard_metrics.metric_key IS 'Identificador da métrica: total_contacts, active_deals, monthly_revenue, etc';
COMMENT ON COLUMN dashboard_metrics.metric_period IS 'Período de agregação: daily, weekly, monthly, all_time';
COMMENT ON COLUMN dashboard_metrics.reference_date IS 'Data de referência da métrica (para métricas históricas)';
COMMENT ON COLUMN dashboard_metrics.metadata IS 'Dados extras em formato JSONB para flexibilidade';
COMMENT ON VIEW v_dashboard_summary IS 'View em tempo real com resumo consolidado do dashboard por organização';
